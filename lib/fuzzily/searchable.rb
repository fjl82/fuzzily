  ...
  module Searchable
    ...
    def make_field_fuzzily_searchable(field, options={})
      ...
      after_save do |record|
        next unless record.respond_to?("saved_change_to_#{field}?".to_sym) ? record.send("saved_change_to_#{field}?".to_sym) : record.send("#{field}_changed?".to_sym)
